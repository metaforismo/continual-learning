...
export interface DurableCommitRequest {
  /** Process-local capability returned by `cursor()` or an earlier successful commit. */
...
function appendDigest(events: readonly MemoryEvent[]): string {
  return contentDigest({
    ...
    events: events.map((event) => ({ seq: event.seq, eventId: event.id, eventDigest: eventDigest(event) })),
  });
}
 ...
 *
 * V1 intentionally replays and re-hashes the complete canonical prefix before every commit. This
 * makes corruption and semantic drift fail closed, but commit cost remains O(N). A later trusted
 * cursor/range layer may optimize that cost without weakening the transaction boundary.
 */
export class SqliteCanonicalLedger {
  ...
  #loadEventsVerified(meta: MetaRow): readonly MemoryEvent[] {
    ...
    for (let index = 0; index < rows.length; index += 1) {
      ...
      if (row === undefined || row.seq !== index + 1) {
        throw new Error(`canonical event sequence gap at ${index + 1}`);
      }
  ...
  #requestSnapshot(request: DurableCommitRequest): {
    ...
    const committedBy = String(request.committedBy);
    const payload = { base, idempotencyKey, committedBy, events, transition, audit };
    ...
    return Object.freeze({ ...payload, requestDigest: contentDigest({ domain: 'cl-durable-request-v1', payload }) });
  }

  #validateRequest(snapshot: ReturnType<SqliteCanonicalLedger['#requestSnapshot']>): void {
    if (
  ...
  status(): DurableLedgerStatus {
    return this.#transaction('read', () => {
      ...
      const audit = this.#db
        ...
        .get(meta.revision) as AuditRow | undefined;
      if (
        audit === undefined ||
        audit.receipt_digest !== receipt.receiptDigest ||
        audit.audit_digest !== receipt.auditDigest
      ) {
        throw new Error('latest durable audit differs from receipt metadata');
      }
  ...
  commit(request: DurableCommitRequest): DurableCommitResult {
    ...
    return this.#transaction('write', () => {
      ...
      return Object.freeze({
        cursor: this.#issueCursor(nextMeta),
        receipt,
  ...
  readRange(fromSeq: number, limit = MAX_RANGE_EVENTS): readonly MemoryEvent[] {
    ...
    return this.#transaction('read', () => {
      const rows = this.#db
        .prepare(`
          SELECT event_json FROM cl_canonical_events
           WHERE seq >= ?
           ORDER BY seq
           LIMIT ?
        `)
        .all(fromSeq, limit) as unknown as readonly { readonly event_json: string }[];
      return Object.freeze(rows.map((row) => JSON.parse(row.event_json) as MemoryEvent));
    });
  }

  loadKernel(): MemoryKernel {
    return this.#transaction('read', () => {
      const events = this.#loadEventsVerified(this.#meta());
      ...
      for (let index = 0; index < receipts.length; index += 1) {
        ...
        try {
          ...
          if (
            ...
            audit.audit_id !== receipt.auditId ||
            audit.audit_digest !== receipt.auditDigest ||
            audit.receipt_digest !== receipt.receiptDigest ||
            auditDigest(JSON.parse(audit.audit_json) as DurableAuditRecord) !== receipt.auditDigest
          ) {
            errors.push(`audit/receipt mismatch at revision ${receipt.revision}`);
          }
          if (
            receipt.previousReceiptDigest !== previousReceipt ||
            receipt.baseChainDigest !== previousChain ||
            receipt.appendFromSeq !== previousToSeq + 1 ||
            receipt.appendToSeq < receipt.appendFromSeq ||
          ...
          const eventRows = this.#db
            .prepare('SELECT event_json FROM cl_canonical_events WHERE seq BETWEEN ? AND ? ORDER BY seq')
            .all(receipt.appendFromSeq, receipt.appendToSeq) as unknown as readonly {
            readonly event_json: string;
          }[];
          const events = eventRows.map((event) => JSON.parse(event.event_json) as MemoryEvent);
          if (appendDigest(events) !== receipt.appendDigest) {
            errors.push(`receipt append digest mismatch at revision ${receipt.revision}`);
          }
          ...
          errors.push(
            error instanceof Error
              ? error.message
              : `receipt audit failed at revision ${row.revision}`,
          );
        }
      }
      if (
